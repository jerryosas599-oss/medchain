...
const { hashRecord, verifyRecord, buildChainedHash } = require('../utils/hash');
const { authenticate, authorize } = require('../middleware/auth');
const { getLastLogHash } = require('./auth');
...
router.get('/:id/verify', authenticate, async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM health_records WHERE id=$1', [req.params.id]);
